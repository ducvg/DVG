...
namespace DVG.Timers
{
	...
	public static class TimerExtension
	{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static Timer OnDispose(this Timer timer, Action onStopAction)
		{
			ref var managedData = ref timer.DataStorage.GetManagedDataRef(timer);
			managedData.DisposeActionContext = null;
			managedData.OnDisposeAction = onStopAction;
			return timer;
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static Timer OnDispose<T>(this Timer timer, 
			T context, Action<T> onStopAction)
		{
			ref var managedData = ref timer.DataStorage.GetManagedDataRef(timer);
			managedData.DisposeActionContext = context;
			managedData.OnDisposeAction = onStopAction;
			return timer;
		}
		...
		{
			ref var managedData = ref timer.DataStorage.GetManagedDataRef(timer);
			managedData.bindOwnerCancellationToken = owner.destroyCancellationToken;
			return timer;
		}
	}
}